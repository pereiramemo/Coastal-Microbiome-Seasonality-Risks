...
```{r}
ABUND_workable_long <- read_tsv("../data/asvs/samo_vs_tara_workable.tsv.gz", show_col_types = FALSE)
SAMO_METADATA <- read_tsv("../data/metadata/samo_metadata_workable.tsv", show_col_types = FALSE)
TARA_METADATA <- read_tsv("../data/metadata/tara_metadata_workable.tsv", show_col_types = FALSE)
```
...
samo_samples <- SAMO_METADATA$amplicon_sample_name 
...
samo_vs_tara_diss_df_ext <- samo_vs_tara_diss_df %>%
  ...
  select(Date, amplicon_sample_name, Season, Temperature_C),
         by = c("samo_sample" = "amplicon_sample_name")) %>%
