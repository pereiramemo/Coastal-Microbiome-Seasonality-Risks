...
```{r}
ABUND <- read_tsv("../data/asvs_workable.tsv", show_col_types = FALSE) %>%
...
ctd_coords$segment_xend[last_i] <- ABUND_norm_nmds_ext %>% filter(Season == "Summer") %>% pull(MDS1) %>% mean()
ctd_coords$segment_yend[last_i] <- ABUND_norm_nmds_ext %>% filter(Season == "Summer") %>% pull(MDS2) %>% mean()
```
