...
`METADATA`:  metadata for SAMO samples.  
`KO_class.csv` custom KO classification into functional categories.  
`KO_desc.csv`  KO pathway description.  

```{r}
KO_TBL <- read_tsv("../data/opus/opus2ko_non_rare_long.tsv.gz", show_col_types = FALSE)
METADATA <- read_tsv("../data/metadata/samo_metadata_workable.tsv", show_col_types = FALSE)
KO_CLASS <- read_tsv("../data/kofam/KO_class.csv", show_col_types = FALSE)
KO2DESC <- read_tsv("../data/kofam/KO_desc.csv", show_col_types = FALSE)
```
...
KO_TBL_collapsed_ext <- left_join(x = KO_TBL_collapsed, 
                                  y = METADATA, 
                                  by = c("sample_name"="metagenome_sample_name"))
```
...
### **7. Define variables to run IndVal over iterated rarefactions**
```{r}
# Set the number of cores to use for parallel processing; 
# this should be adjusted to the local computational capacitites.
n_cores <- 44
...
### **8. Run IndVal over iterated rarefactions**
...
### **9. Select KOs with a 80% consistency or greater**
```{r}
